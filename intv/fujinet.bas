...
' ---------------------------------------------------------------------------
' JSON channel mode (ISS tracker addition). The mailbox bridge forwards any
' device/command verbatim (pico fuji_mailbox_service()), and the firmware's
' fujibus network device handles these three commands in rs232_process()
' (fujinet-firmware lib/device/rs232/network.cpp), so no firmware changes
' are needed:
'
'   $FC CHANNEL_MODE  two params, mode rides in param(1); JSON = 1
'   $50 PARSE         no params; parses the downloaded body FujiNet-side
'   $51 QUERY         no params; the JSON path is the TX payload. The
'                     query result is appended to the channel's receive
'                     buffer: a following net_status reports its length
'                     in #net_avail and net_read drains it into FN_RX.
' ---------------------------------------------------------------------------
CONST NETCMD_CHANNEL_MODE = $FC
CONST NETCMD_PARSE = $50
CONST NETCMD_QUERY = $51
CONST CHANMODE_JSON = 1

' net_chanmode: switch the open channel's mode. Caller sets cm_mode
' (CHANMODE_JSON). fn_ok as usual.
DIM cm_mode
net_chanmode: PROCEDURE
    mb_dev = NET_DEVICEID
    mb_cmd = NETCMD_CHANNEL_MODE
    mb_nparam = 2
    pm_i = 0 : pm_size = 1 : #pm_val = 0 : GOSUB fn_param
    pm_i = 1 : pm_size = 1 : #pm_val = cm_mode : GOSUB fn_param
    #fn_txlen = 0
    GOSUB fn_transact
END

' net_parse: parse the downloaded body as JSON. fn_ok as usual.
net_parse: PROCEDURE
    mb_dev = NET_DEVICEID
    mb_cmd = NETCMD_PARSE
    mb_nparam = 0
...
' net_query: set the JSON query. Caller stages the path at FN_TX (with
' #fn_txlen set) beforehand, exactly like net_open's devicespec. fn_ok as
' usual; read the result via net_status + net_read.
net_query: PROCEDURE
    mb_dev = NET_DEVICEID
    mb_cmd = NETCMD_QUERY
    mb_nparam = 0
