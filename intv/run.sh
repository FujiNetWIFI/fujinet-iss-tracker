#!/bin/sh
# run.sh -- build (if needed) and launch the ISS tracker in the
# FujiNet-patched jzIntv, connected to a fujinet-firmware instance over BoIP.
#
...
# last built.
if [ ! -f iss.rom ] || [ -n "$(find . -maxdepth 1 -name '*.bas' -newer iss.rom)" ]; then
    echo "Building iss.rom..."
    make
fi
